...
var ContinuousVisualization = function(canvas, context) {
	...
	this.drawEntities = function(idx, p) {
		...
		if (p.type === "agent")
		{
			...
			ctx.fillStyle = "white";
			ctx.font = "10px Arial"
			ctx.fillText(idx + ":" + p.id , cx-3, cy-10)
			if (p.selected) {
			...
			if (p.selected)
			{
				...
				// el.src = '/images/id' + p.id + '.svg'
				el = document.getElementById('print_info')
				el.innerText = ''
				for (const name of p.print_info) {
					el.innerText+= name + ': ' + p[name] + '\n'
				}
				el.innerText += '----EVOLVED PARAMS----\n'

				for (const [key, value] of Object.entries(p.evol_pars)) {
					el.innerText+= key + ': ' + parseFloat(value.toFixed(2)) + '\n'
				}
...
var ForagingCanvas = function(penv_width, penv_height, name_sim, model_load) {
	...
	$("#leftCol").append(canvas);
	let context = canvas.getContext("2d");
	let canvasDraw = new ContinuousVisualization(canvas, context);

	var info_tag = "<p>Step <span id='step_count'>0 </span> Population <span id='popcount'>0</span></p>"
	$("#rightCol").append($(info_tag)[0])
	...
	add_checkbox("render_canvas", "Render CANVAS", "leftCol")
	add_checkbox("render_fov", "Render FOV", "leftCol", false)
	add_checkbox("render_bars", "Render BARS", "leftCol", false)

	var output_tag = "<pre>...</pre>"
	var output = $(output_tag)[0]
	$("#rightCol").append(output)
	...
	this.render = function(data) {
		...
		document.getElementById("popcount").innerHTML=data["pop_count"]
		document.getElementById("step_count").innerHTML=data["step"] + "\tFood Count " + data["food_count"] + "\t"
